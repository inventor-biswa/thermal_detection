...
DATA_STREAM = True  # Stream JSON thermal data over serial for ML collection
...
while True:
    ...
    # Stream raw 8x8 thermal data as JSON for ML data collection
    if DATA_STREAM and not DISPLAY_HOLD:
        frame_out = {
            "ts": mkr_t2,
            "max_c": round(float(v_max), 2),
            "min_c": round(float(v_min), 2),
            "avg_c": round(float(v_ave), 2),
            "grid": [list(row) for row in sensor]
        }
        print("THERMAL:" + json.dumps(frame_out))
